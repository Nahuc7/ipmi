...
void draw() {
  ...
  if (estado == "pantalla1") {
    ...
    drawText("Bienvenidos", width / 2, height / 2 - 30); 
    if (tiempo >= 5 * 60) {
      estado = "pantalla2";
      tiempo = 0;
      alpha = 0; 
    ...
    colorFinal = color(255, 255, 255);
    fill(lerpColor(colorInicial, colorFinal, t));
    drawText("A la familia Shelby", width / 2, height / 2); 
    if (tiempo >= 12 * 60) { 
      estado = "pantalla3";
      tiempo = 0;
      alpha = 0; 
    ...
    colorFinal = color(255, 255, 255);
    fill(lerpColor(colorInicial, colorFinal, t));
    drawText("Luego de la posguerra britanica, volvimos a Birmingham, para tomar lo que nos pertenece, y asi poder hacer nuestro, negocio por las buenas, o por las malas", width / 2, height / 2 - 30); 
    if (tiempo >= 15 * 60) { 
      estado = "pantalla4";
      tiempo = 0;
      alpha = 0; 
    ...
    colorFinal = color(255, 255, 255);
    fill(lerpColor(colorInicial, colorFinal, t));
    drawText("Asi que más te vale, no cruzarte con nosotros", width / 2, height / 2 - 30); 
    if (tiempo >= 5 * 60) { 
      estado = "pantalla5";
      tiempo = 0;
      alpha = 0; 
...
void drawText(String text, float x, float y) {
  ...
  float rectHeight = lines.length * lineSpacing;
  textAlign(CENTER, CENTER);
  for (int i = 0; i < lines.length; i++) {
    stroke(0);
    strokeWeight(2);
    text(lines[i], x, y + i * lineSpacing - (lines.length - 1) * lineSpacing / 2); 
}
}
